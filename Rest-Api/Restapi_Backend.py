...
async def insert_record(job_id:str,job_name:str,location:str,description:str,skills:str):
    ...
    if val == (0,):
        query = Job_Table.insert().values(job_id=job_id, job_name=job_name, location=location,
...
async def update_record(job_id:str,job_name:str,location:str,description:str,skills:str):
    query1 = select([count(1)]).where((Job_Table.c.job_id == job_id))
    val = await database.fetch_one(query1)
    if val == (0,):
